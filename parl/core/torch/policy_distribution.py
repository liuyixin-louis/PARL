...
#         """
#         mean_shape = torch.to_tensor(self.mean.shape, dtype='int64')
#         random_normal = torch.normal(shape=mean_shape)
...
class DiagGaussianDistribution(PolicyDistribution):
    ...
    def logp(self, actions):
        ...
        actions_shape = torch.to_tensor(self.actions.shape, dtype='float32')
        pi_item = 0.5 * np.log(2.0 * np.pi) * actions_shape[1]
